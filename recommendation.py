                ...
                if row[0] in authors.values():
                    for k, v in authors.items():
                        if v== row[0]:
                            ...
                            csv_dict[k][row[3]]['Disgust'] = float(row[5])
                            csv_dict[k][row[3]]['Fear'] = float(row[6])
                            csv_dict[k][row[3]]['Joy'] = float(row[7])
                            csv_dict[k][row[3]]['Sadness'] = float(row[8])
                            csv_dict[k][row[3]]['Surprise'] = float(row[8])
                    ...
                    csv_dict[line_count][row[3]]['Disgust'] = float(row[5])
                    csv_dict[line_count][row[3]]['Fear'] = float(row[6])
                    csv_dict[line_count][row[3]]['Joy'] = float(row[7])
                    csv_dict[line_count][row[3]]['Sadness'] = float(row[8])
                    csv_dict[line_count][row[3]]['Surprise'] = float(row[8])
                line_count += 1
    return csv_dict
